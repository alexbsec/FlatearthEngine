...
LRESULT CALLBACK win32ProcessMessage(HWND hwnd, uint32 msg, WPARAM wParam, LPARAM lParam);

namespace flatearth {
namespace platform {

	Platform::Platform(const string& application_name, sint32 x, sint32 y, sint32 width, sint32 height)
		...
		InternalState* inStatePtr = get_unique_void_ptr<InternalState>(_state->internalState);
		...
		HWND handle = CreateWindowExA(windowExStyle, "flatearth_window_class", application_name.c_str(), windowStyle, windowX, windowY, windowWidth, windowHeight, 0, 0, inStatePtr->hInstance, 0);
...
void Platform::ConsoleWrite(const string& message, uchar color) {
	...
	WriteConsoleA(GetStdHandle(STD_OUTPUT_HANDLE), sOut.c_str(), (DWORD)length, numberWritten, 0);
}
